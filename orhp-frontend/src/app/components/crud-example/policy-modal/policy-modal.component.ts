...
export class PolicyModalComponent {
  ...
  public onClickSubmit(data: any) {
    console.log('submit', this.policy.id, data.monthlyPremium.value, data.deductible.value, this.selectedUsers);
    this.policyService.updatePolicy(this.policy.id, data.monthlyPremium.value, data.deductible.value, this.selectedUsers)
    .subscribe((res) => {
      console.log(res);
      this.loader = false;
